...
The broader repository contains real-provider paths, durable PostgreSQL scheduling, human review/adjudication, encrypted evidence storage, OIDC/project controls, telemetry, backup/recovery, hardened deployment material, GUI/operator controls, and certification orchestration. The package version remains `0.1.0`, but semantic version alone neither proves immaturity nor certifies production readiness.
...
| **Provisional** | Implementation exists, but calibration, reference work, policy approval, or another evidence requirement is incomplete. |
...
| Persona analyst view project isolation | **Implemented** | Analyst views now reject unscoped or cross-project canonical reports before copying metrics/artifact lineage. Higher-level authorization still remains a caller/API responsibility. |
| Executive persona support/uncertainty aggregates | **Provisional** | Current `build_executive_summary` still uses provisional placeholder aggregate support/uncertainty values because `CanonicalReport` does not yet define authoritative aggregate contracts for them. Do not cite those fields as measured release evidence. |
...
| Parquet report export | **Implemented as optional capability** | Requires `pyarrow`; missing support is an explicit error rather than a zero-byte artifact. Parquet rows carry report identity/hash plus metric rows. `pyarrow` is not a default package dependency. |
| Content-addressed local evidence | **Implemented / local-lane exercised** | Strong development/CI traceability; local filesystem storage alone is not managed production immutability. |
| Encrypted evidence store | **Implemented** | AES-256-GCM envelope-encryption/retention interfaces exist; development `LocalKMSClient` is not a production KMS authority. |
...
| GUI loopback boundary | **Implemented / integrated in supported launcher** | Direct non-loopback binding is rejected; authenticated remote proxying is a deployment responsibility. |
| GUI runtime overlays (`ux4`/`ux5`/`ux6`) | **Implemented / integrated** | The supported server injects these layers into baseline `index.html` before serving `/`; they are active runtime assets even though the baseline HTML does not contain static script tags for them. |
...
| Backup/recovery | **Implemented modules/workflows** | A real recovery claim requires executed restore/PITR/object-reconciliation evidence. |
...
It does **not** exercise every provider, production scheduler, external KMS/secret manager, organizational IdP, private egress boundary, multi-user review operation, backup service, production certificate, or target deployment.
...
`src/wilson_eval3ngine/ui/views.py` derives release status and critical blocks from the canonical report, but its aggregate support and uncertainty percentages remain provisional constants because `CanonicalReport` does not yet provide an authoritative aggregate support/uncertainty contract. The fields remain visible as an implementation gap rather than being presented as measured values.

### Analyst/reviewer scope

The analyst helper now enforces that the canonical report's project matches the authorized project argument and rejects missing project scope. That closes the local view-construction relabelling gap, but authorization must still be enforced at API/service boundaries and backed by real identity/project policy. The reviewer redaction helper is intentionally described as baseline pattern masking rather than a production DLP engine.

### Report/export boundaries

Cross-format hash reconciliation now checks that JSON, CSV, and HTML representations carry the exact canonical hash instead of returning unconditional success. This verifies a shared representation identifier; it does not independently prove semantic equality of every serialized field. Parquet export requires the optional `pyarrow` package and fails explicitly if unavailable, avoiding a zero-byte artifact that could be mistaken for success.
...
The supported GUI is protected primarily by loopback binding and optional access-control composition, not by pretending every local desktop session is a production multi-user security boundary. A remotely exposed GUI requires an authenticated/authorized TLS proxy and target-deployment validation.
...
> **Wilson Eval3ngine `0.1.0` is an active evidence-first LLM evaluation platform in pre-production assurance. The deterministic local foundation lane is functional for development/CI, while the repository also contains substantial provider, scheduling, review, security, evidence, operations, reporting, and certification capabilities. Several explicitly documented statistical/persona-view areas remain provisional, and production certification is evidence-dependent for the exact release/deployment being approved.**

See [Architecture](ARCHITECTURE.md) for component relationships, [Getting Started](GETTING_STARTED.md) for the first safe run, and [GUI & Evidence Guide](GUI_AND_EVIDENCE_GUIDE.md) for the operator/evidence model.
